...
AbstractSQLExpression
                     ...
                     * AlinousLang::join() {AbstractSQLExpression* left = nullptr;
        AbstractSQLExpression* right = nullptr;
    if (!hasError) {
    left = joinTarget();
    }
...
AbstractSQLExpression                     * AlinousLang::joinTarget() {AbstractSQLExpression* target = nullptr;
    if (!hasError) {
    switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
    case IDENTIFIER:{
      if (!hasError) {
