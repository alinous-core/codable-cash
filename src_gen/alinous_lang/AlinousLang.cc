...
unsigned int jj_la1_0[] = { 0x0, 0x0, 0x0, 0x0, 0x0, 0x200000, 0x200000, 0x0,
		0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x140200, 0x140200,
		0x140200, 0x0, 0x140000, };
unsigned int jj_la1_1[] = { 0x1000000, 0x2000000, 0x80, 0x0, 0x0, 0x4, 0x800,
		0x0, 0x100, 0xe0000000, 0xe0000000, 0x0, 0x80000, 0x80, 0x10, 0x4,
		0x100, 0x80004, 0x4, 0x0, 0x0, 0x0, };
unsigned int jj_la1_2[] = { 0x0, 0x0, 0x0, 0x70000, 0x100000, 0x0, 0x0, 0x70000,
		0x0, 0x10000003, 0x10000003, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80000000,
		0x80000000, 0x80000000, 0x80000000, 0x0, };
unsigned int jj_la1_3[] = { 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
		0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0x1, 0x1, 0x1, 0x0, };
unsigned int jj_la1_4[] = { 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
		0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, };
unsigned int jj_la1_5[] = { 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
		0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x8040000, 0x8040000, 0x40000, 0x0,
		0x0, };
unsigned int jj_la1_6[] = { 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
		0x0, 0x4, 0x0, 0x0, 0x0, 0x0, 0x0, 0x4, 0x0, 0x0, 0x0, 0x0, };

/** Constructor with user supplied TokenManager. */

CompilationUnit * AlinousLang::compilationUnit() {
...
PackageDeclare * AlinousLang::packageDeclare() {
	PackageDeclare* pkg = nullptr;
...
PackageNameDeclare * AlinousLang::packageNameDeclare() {
...
ClassDeclare * AlinousLang::classDeclare() {
	ClassDeclare* clazz = nullptr;
	Token* cls = nullptr;
	Token* name = nullptr;
...
ClassDeclareBlock * AlinousLang::classDeclareBlock() {
	ClassDeclareBlock* block = nullptr;
	Token* begin, *end;
		...
		while (!hasError) {
			...
			if (!hasError) {
				...
				default:
					jj_la1[5] = jj_gen;
					jj_consume_token(-1);
					errorHandler->handleParseError(token, getToken(1),
							__FUNCTION__, this), hasError = true;
				}
			}
			if (!hasError) {
				_static = false;
...
MethodDeclare * AlinousLang::methodDeclare(AccessControlDeclare* ctrl,
		bool _static, AbstractType* type, Token* identifier) {
	...
	if (!hasError) {
		...
		default:
			jj_la1[6] = jj_gen;
			jj_consume_token(-1);
			errorHandler->handleParseError(token, getToken(1), __FUNCTION__,
					this), hasError = true;
		}
	}
	if (!hasError) {
...
MemberVariableDeclare * AlinousLang::memberVariableDeclare(
		AccessControlDeclare* ctrl, bool _static, AbstractType* type,
		Token* identifier) {
...
AccessControlDeclare * AlinousLang::accessControlDeclare() {
	Token* t = nullptr;
...
ArgumentsListDeclare * AlinousLang::argumentsListDeclare() {
...
ArgumentDeclare * AlinousLang::argumentDeclare() {
...
AbstractType * AlinousLang::typeDeclare() {
...
ShortType * AlinousLang::shortType() {
...
StringType * AlinousLang::stringType() {
...
StatementBlock * AlinousLang::statementBlock() {
...
IfStatement * AlinousLang::ifStatement() {
...
AbstractExpression * AlinousLang::expression() {
	AbstractExpression* exp = nullptr;
	if (!hasError) {
		if (!hasError) {
			exp = bitReverseExpression();
		}
	}
	if (!hasError) {
...
AbstractExpression * AlinousLang::bitReverseExpression() {
	Token* t = nullptr;
...
AbstractExpression * AlinousLang::notExpression() {
	Token* t = nullptr;
...
AbstractExpression * AlinousLang::castExpression() {
	Token* t = nullptr;
...
AbstractExpression * AlinousLang::memberReferenceExpression() {
...
AbstractExpression * AlinousLang::arrayReferenceExpression() {
...
AbstractExpression * AlinousLang::functionCallExpression() {
...
void AlinousLang::functionCallExpressionArg(FunctionCallExpression* exp) {
...
AbstractExpression * AlinousLang::primitive() {
	AbstractExpression* exp = nullptr;
	if (!hasError) {
		switch ((jj_ntk == -1) ? jj_ntk_f() : jj_ntk) {
		case L_PARENTHESIS: {
...
ParenthesisExpression * AlinousLang::parenthesisExpression() {
...
VariableIdentifier * AlinousLang::variableIdentifier() {
...
AbstractExpression * AlinousLang::literal() {
...
NullLiteral * AlinousLang::nullLiteral() {
...
NumberLiteral * AlinousLang::numberLiteral() {
...
BooleanLiteral * AlinousLang::booleanLiteral() {
...
LiteralExpression * AlinousLang::literalExpression() {
...
Token * AlinousLang::jj_consume_token(int kind) {
	...
	errorHandler->handleUnexpectedToken(kind, image.substr(1, image.size() - 2),
			getToken(1), this);
	hasError = true;
	return token;
}
...
void AlinousLang::parseError() {
	fprintf(stderr, "Parse error at: %d:%d, after token: %s encountered: %s\n",
			...
			addUnicodeEscapes(token->image).c_str(),
